...
bool isRooted() {
    ...
    for (auto p : paths) if (access(p, F_OK)==0) { LOGI("Root found: %s", p); return true; }
    return false;
}
...
bool isFrida() {
    ...
    std::string line;
    while (std::getline(maps, line)) if (line.find("frida")!=std::string::npos
        || line.find("gum-js-loop")!=std::string::npos) { LOGI("Frida detected"); return true; }
    return false;
}

// Debugger detection
bool isDebug() {
    if (ptrace(PTRACE_TRACEME,0,0,0)==-1) { LOGI("Debugger attached"); return true; }
    return false;
}
...
bool isProxy() {
    ...
    if (p && (strstr(p,"127.")||strstr(p,"localhost"))) { LOGI("Proxy set: %s",p); return true; }
    return false;
}
...
bool isBurpCanary() {
    ...
    while ((d=readdir(dir))!=nullptr) {
        ...
        if (n.find("burp")!=std::string::npos || n.find("canary")!=std::string::npos) {
            closedir(dir); LOGI("Burp/Canary pkg found: %s", n.c_str()); return true;
        }
    }
    closedir(dir);
    return false;
